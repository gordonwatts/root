...
static int G__asm_gen_stvar(size_t G__struct_offset, int ig15, int paran, G__var_array* var, const char* item, size_t store_struct_offset, int var_type, G__value* presult);
//--
//--
...
static void G__class_2nd_decl(G__var_array* var, int ig15)
{
   ...
   if (G__struct.iscpplink[tagnum] == G__CPPLINK) {
      ...
      if (G__cpp_aryconstruct) {
         for (size_t i = G__cpp_aryconstruct - 1; i >= 0; --i) {
...
static void G__class_2nd_decl_i(G__var_array* var, int ig15)
{
   ...
   if (var->varlabel[ig15][1] /* number of elements */ || var->paran[ig15]) {
      ...
      size_t pinc = var->varlabel[ig15][1] /* number of elements */;
#ifdef G__ASM_DBG
      if (G__asm_dbg) {
   ...
   switch (G__var_type) { \
      ...
      case 'p': \
         ...
         if (paran >= var->paran[ig15]) { \
            ...
            result.ref = (long) (G__struct_offset + var->p[ig15] + (linear_index * SIZE)); \
      ...
      case 'v': \
         ...
         switch (var->reftype[ig15]) { \
            case G__PARANORMAL: \
               ...
               result.ref = *((size_t*) (G__struct_offset + var->p[ig15] + (linear_index * G__LONGALLOC))); \
               *((CASTTYPE*) result.ref) = (CASTTYPE) CONVFUNC(result); \
            ...
            case G__PARAP2P: \
               ...
               if (paran > var->paran[ig15]) { \
                  ...
                  result.ref = *(((size_t*) (*((size_t *) address))) + secondary_linear_index); \
                  *((CASTTYPE*) result.ref) = (CASTTYPE) CONVFUNC(result); \
...
G__value G__letvariable(G__FastAllocString &item, G__value expression, G__var_array* varglobal, G__var_array* varlocal, G__DataMemberHandle &member)
{
   ...
   size_t G__struct_offset = 0L;
   char* tagname = 0;
   size_t membername = -1;
   int varhash = 0;
   size_t address = 0L;
   size_t store_struct_offset = 0L;
   int store_tagnum = 0;
   ...
   if (var) {
      ...
      if (
         ...
         if (
            ...
            if (
               ...
               size_t protect_struct_offset = G__store_struct_offset;
               int done = 0;
      ...
      linear_index = 0;
      {
         ...
         // tmp = B*C*D
         size_t tmp = var->varlabel[ig15][0] /* stride */;
...
//______________________________________________________________________________
void G__letpointer2memfunc(G__var_array* var, int paran, int ig15, const char* item, size_t linear_index, G__value* presult, size_t G__struct_offset)
{
   // -- FIXME: Describe me!
   switch (G__var_type) {
      case 'p':
...
//______________________________________________________________________________
void G__letautomatic(G__var_array* var, int ig15, size_t G__struct_offset, size_t linear_index, G__value result)
{
   // -- FIXME: Describe me!
   if (isupper(var->type[ig15])) {
...
G__value G__letstructmem(int store_var_type, G__FastAllocString& varname,
                         size_t membernameoffset, G__FastAllocString& result7,
...
//______________________________________________________________________________
void G__letstruct(G__value* result, size_t linear_index, G__var_array* var, size_t ig15, const char* item, int paran, size_t G__struct_offset)
{
...
//______________________________________________________________________________
void G__letstructp(G__value result, size_t G__struct_offset, int ig15, size_t linear_index, G__var_array* var, int paran, const char* item, G__value* para, size_t secondary_linear_index)
{
   // -- FIXME: Describe me!
   int baseoffset = 0;
   switch (G__var_type) {
...
//______________________________________________________________________________
static int G__asm_gen_stvar(size_t G__struct_offset, int ig15, int paran, G__var_array* var, const char*
#ifdef G__ASM_DBG
item
#endif
, size_t store_struct_offset, int var_type, G__value* /*presult*/)
{
   // -- FIXME: Describe me!
   //
...
inline void G__get_pvar(CONVFUNC f, char TYPE, char PTYPE, struct G__var_array* var, int ig15, size_t G__struct_offset, int paran, G__value para[G__MAXVARDIM], size_t linear_index, size_t secondary_linear_index, G__value* result)
{
   switch (G__var_type) {
      case 'v':
      ...
      case 'P':
         ...
         if (var->paran[ig15] == paran) {
            ...
            size_t address = G__struct_offset + var->p[ig15] + (linear_index * G__LONGALLOC);
            if (var->reftype[ig15] == G__PARANORMAL) {
               G__letint(result, PTYPE, (size_t)((CASTTYPE*)(*(size_t*)address) + secondary_linear_index));
            }
            else {
...
// TODO: G__LONGALLOC is not the right hting here - needs something liek ptr alloc or similar.
   ...
   switch (G__var_type) { \
      case 'v': \
         ...
         switch (var->reftype[ig15]) { \
            ...
            case G__PARAP2P: \
               if (var->paran[ig15] < paran) { \
                  size_t address = G__struct_offset + var->p[ig15] + (linear_index * G__LONGALLOC); \
                  result.ref = *(size_t*) (((CASTTYPE*) (*(size_t*) address)) + secondary_linear_index); \
                  if (result.ref) { \
                     CONVFUNC(&result, TYPE, (CONVTYPE) (*(CASTTYPE*) result.ref)); \
                  } \
               } \
                  ...
                  result.ref = *(size_t*)(G__struct_offset + var->p[ig15] + (linear_index * G__LONGALLOC)); \
                  G__letint(&result, PTYPE, *(size_t*)(*(size_t*) (G__struct_offset + var->p[ig15] + (linear_index * G__LONGALLOC)))); \
               } \
               break; \
         } \
         break; \
      case 'P': \
         ...
         if (var->paran[ig15] == paran) { \
            ...
            if (var->reftype[ig15] == G__PARANORMAL) { \
               ...
               G__letint(&result, PTYPE, (size_t)((size_t*)(*(size_t*)address) + secondary_linear_index)); \
               result.obj.reftype.reftype = G__PARAP2P; \
            } \
         } \
         else { \
      ...
      default : \
         ...
         if (paran == var->paran[ig15]) { \
            /* MyType* var[ddd]; MyType* v = var[xxx]; */ \
            result.ref = (size_t)(G__struct_offset + var->p[ig15] + (linear_index * G__LONGALLOC)); \
            G__letint(&result, PTYPE, *((size_t*) result.ref)); \
         } \
         else if (paran > var->paran[ig15]) { \
            /* -- Pointer to array reimplementation. */ \
            /* MyType* var[ddd];  v = var[xxx][yyy]; */ \
            ...
            if (var->reftype[ig15] == G__PARANORMAL) { \
               ...
               /* Pointer to array reimplementation. */ \
               result.ref = (size_t)((size_t*)(*(size_t*)address) + secondary_linear_index); \
               G__letint(&result, PTYPE, (size_t)((CASTTYPE*)(*((size_t*)result.ref)))); \
               if (var->reftype[ig15] > G__PARAP2P) { \
                  result.obj.reftype.reftype = var->reftype[ig15] - 1; \
               } \
            } \
               ...
               /* -- Start doing pointer arithmetic. */ \
               result.ref = (size_t)((size_t*)(*(size_t*)address) + para[0].obj.i); \
               for (int ip = 1; ip < (paran - 1); ++ip) { \
                  result.ref = (size_t)((size_t*)(*(size_t*)result.ref) + para[ip].obj.i); \
               } \
               result.obj.reftype.reftype = var->reftype[ig15] - paran + var->paran[ig15]; \
               /**/ \
               /**/ \
               ...
               switch (result.obj.reftype.reftype) { \
                  case G__PARANORMAL: \
                     result.ref = (size_t)((CASTTYPE*)(*((size_t*)result.ref)) + para[paran-1].obj.i); \
                     CONVFUNC(&result, TYPE, *((CASTTYPE*)result.ref)); \
                     break; \
                  case 1: \
                     result.ref = (size_t)((size_t*)(*((size_t*)result.ref)) + para[paran-1].obj.i); \
                     G__letint(&result, PTYPE, *((size_t*)result.ref)); \
                     result.obj.reftype.reftype = G__PARANORMAL; \
                     break; \
                  default: \
            ...
            /* FIXME: This is a syntax error if (var->paran[ig15] - paran) > 1. */ \
            if (G__struct_offset) { \
               result.ref = (size_t)(G__struct_offset + var->p[ig15]);         \
            } else { \
               result.ref = (size_t)(&var->p[ig15]); \
            } \
            G__letint(&result, PTYPE, *((size_t*) result.ref)); \
         } \
         break; \
...
#define G__OLDIMPLEMENTATION965 // FIXME: Remove this?
   ...
   if (islower(result.type) && (objptr == 2)) {
      ...
      size_t store_struct_offsetB = G__store_struct_offset;
      G__tagnum = result.tagnum;
      G__store_struct_offset = result.obj.i;
...
//______________________________________________________________________________
struct G__var_array* G__searchvariable(char* varname, int varhash, G__var_array* varlocal, G__var_array* varglobal, size_t* pG__struct_offset, size_t* pstore_struct_offset, int* pig15, int isdecl)
{
   // -- FIXME: Describe me!
   struct G__var_array* var = 0;
   int ilg = 0;
   int in_memfunc = 0;
   size_t scope_struct_offset = 0L;
   int scope_tagnum = 0;
   int basen = 0;
   int isbase = 0;
   int accesslimit = 0;
      ...
      switch (var->type[ig15]) {
         case 'u': {
            ...
            if (G__struct.iscpplink[G__tagnum] == G__CPPLINK) {
               ...
               int size = G__struct.size[G__tagnum];
               size_t i = var->varlabel[ig15][1] /* number of elements */;
               if (!i) {
                  i = 1;
               }
               --i;
