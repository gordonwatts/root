...
$cintbin = "C:\Users\Gordon\Documents\Code\root\vc12\bin\DebugFake"
...
#& "$($cintbin)\cint.exe" -?
& "$($cintbin)\cint.exe" -Z0 -n vc12strm.cxx -NG__stream -D__MAKECINT__ -D_WIN32 -c-1 -I . -I ..\..\include $includes 2>&1 | add-content -path $logfile

if (test-path vc12strm.cxx) {
  cp vc12strm.* ..\..\src\dict
}
